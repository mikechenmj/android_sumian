package com.sumian.app.widget.histogram;
...
import com.sumian.app.network.response.SleepDuration;
...
public class SleepHistogramView extends View {
    ...
    private int mYdefaultProgress;//y 默认最大值
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        ...
        this.mYMaxProgress = typedArray.getInt(R.styleable.SleepHistogramView_y_max_progress, 8);
        this.mYdefaultProgress = mYMaxProgress;
...
//    }
    ...
    public void addSleepData(List<SleepDuration> sleepDurations) {
        ...
        if (sleepDurations != null && !sleepDurations.isEmpty()) {
            int scale = 3600 * mYdefaultProgress;//默认显示8小时
        ...
        this.mYMaxProgress = maxSleepDuration > mYdefaultProgress ? maxSleepDuration : mYdefaultProgress;//计算进度最大值自增
        int growthProgress = maxSleepDuration > mYdefaultProgress ? (maxSleepDuration - mYdefaultProgress) >> 1 : 0;//计算进度比原先值增加比例
