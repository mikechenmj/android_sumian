...
public final class FileHelper {
    ...
    private static String formatFileName(String fileName) {
        ...
        //初始化新文件名
        return String.format("%s%s%s%s", fileName, TimeUtil.formatDate2FileName(INSTANCE.mUnixTime), FILE_CONNECTOR, FILE_SUFFIX);
    }
    ...
    public static void updateFileDate(long unixTime) {
        INSTANCE.mUnixTime = unixTime;

        INSTANCE.mFileThread = new FileThread();
        INSTANCE.mFileThread.setUnixTime(unixTime);

        AppOperator.runOnThread(INSTANCE.mFileThread);
    }

    public static void closeUploadThread() {
        FileThread fileThread = INSTANCE.mFileThread;
        if (fileThread == null) return;
        INSTANCE.mFileThread.release();
    }
