package com.sumian.app.command;
...
public final class BlueCmd {
    ...
    public static byte[] cUserInfo() {
        ...
        bytes[2] = 0x04;
        bytes[3] = (byte) (AppManager.getAccountModel().getUserInfo() != null ? AppManager.getAccountModel().getUserInfo().getGenderType() : 0xff);//性别特征
        bytes[4] = (byte) (AppManager.getAccountModel().getUserInfo() != null ? AppManager.getAccountModel().getUserInfo().getFormatAge() : 0xff);//年龄
        bytes[5] = (byte) (AppManager.getAccountModel().getUserInfo() != null ? AppManager.getAccountModel().getUserInfo().getFormatBmi() : 0xff);//身高体重比
        bytes[6] = (byte) (AppManager.getAccountModel().getUserInfo() != null ? AppManager.getAccountModel().getUserInfo().getInsomnia() : 0xff);//失眠程度
        return bytes;
    }
}
