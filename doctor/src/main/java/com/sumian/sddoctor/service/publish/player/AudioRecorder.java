package com.sumian.sddoctor.service.publish.player;
...
public class AudioRecorder implements AudioRecord.OnRecordPositionUpdateListener {
    ...
    private List<String> mRecordFiles = new ArrayList<>(0);
    ...
    // after onPauseCallback() is called, this size is written to the header/data chunk in
    // the wave file
    ...
    private AudioRecorder(int audioSource, int sampleRate, int channelConfig, int audioFormat) {
        try {
            ...
            if (bufferSize < AudioRecord.getMinBufferSize(sampleRate,
                ...
                Log.e(TAG, "Increasing buffer size to " + Integer.toString(bufferSize));
    ...
    public static AudioRecorder init() {
        return new AudioRecorder(MediaRecorder.AudioSource.MIC, sampleRates[3], AudioFormat.CHANNEL_IN_STEREO, AudioFormat.ENCODING_PCM_16BIT);
    }
    ...
    private synchronized void writeData() {
        ...
        try {
            ...
            // Log.e(TAG, "buffer size:" + buffer.length + " payloadSize:" + payloadSize);
            if (bSamples == 16) {
                //long len = 0;
...
//				Log.e("amplitude","amplitude:" + result + " time:" + System.currentTimeMillis() + " show volume:" + volume);
    ...
    public synchronized void stop() {
        if (state == State.RECORDING) {
            ...
            Log.e(TAG, "onPauseCallback() called on illegal state");
            state = State.ERROR;
        }
    }
    ...
    public File mergeAudioFile(Context context) {
        String path = FilePathUtil.makeFilePath(context, AUDIO_DIR_PATH, "merge" + System.currentTimeMillis() + AUDIO_SUFFIX_MP3);
        ...
        mRecordFiles.clear();
        if (mixFile.exists()) {
            Log.i(TAG, "file size:" + mixFile.length());
        }
        return mixFile;
    }
