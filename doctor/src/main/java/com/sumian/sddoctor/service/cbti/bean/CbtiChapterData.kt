...
data class CbtiChapterData(
        ...
        @SerializedName("summary_rtf") val summaryRtf: String,
        @SerializedName("index") val index: Int,
        @SerializedName("total_count") val totalCount: Int,//章节数
        @SerializedName("finished_count") val finished_count: Int,//完成数量
        @SerializedName("finished_percent") val finished_percent: Int//完成百分比
) {

    fun formatProgress(): String {
        return when (finished_percent) {
            0 -> {
                "进度：$finished_percent%"
            }
            100 -> {
