...
import com.sumian.sddoctor.service.report.util.TextUtil;
...
public final class TimeUtil {
    ...
    public static long getStartTimeOfTheWeek(long time) {
        time = getStartTimeOfTheDay(time);
        Calendar calendar = getDayStartCalendar(time);
    ...
    public static Calendar getStartTimeCalendarOfTheMonth(long time) {
        time = getStartTimeOfTheDay(time);
        Calendar calendar = getDayStartCalendar(time);
        int dayOfMonth = calendar.get(Calendar.DAY_OF_MONTH);
        rollDay(calendar, -(dayOfMonth - 1));
    ...
    public static long getStartTimeOfTheMonth(long time) {
        return getStartTimeCalendarOfTheMonth(time).getTimeInMillis();
    }
...
//            CharSequence concat = TextUtils.concat(charSequence, " ");
    ...
    public static boolean isInTheSameWeek(long t1, long t2) {
        Calendar c1 = getCalendar(t1);
        Calendar c2 = getCalendar(t2);
        return c1.get(Calendar.YEAR) == c2.get(Calendar.YEAR)
