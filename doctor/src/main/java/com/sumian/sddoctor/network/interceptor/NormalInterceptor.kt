...
class NormalInterceptor private constructor() : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        val builder = chain
                .request()
                .newBuilder()
                .addHeader("Accept-Language", UiUtils.getSystemLanguage())
                .addHeader("Content-Type", "application/json")
                .addHeader("User-Agent", "$userAgent Sumian-Doctor-Android uid:${AppManager.getAccountViewModel().getDoctorInfo().value?.id
                        ?: 0}")
