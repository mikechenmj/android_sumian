...
public class ExamineBatteryView extends View {
    ...
    private RectF mAhClipRectF = new RectF();
    ...
    public ExamineBatteryView(Context context) {
        this(context, null);
    }

    public ExamineBatteryView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ExamineBatteryView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        RectF ahRectF = new RectF(batteryAhLeft, batteryAhTop, batteryAhRight, batteryAhBottom);
...
//    }
    ...
    private void drawPercentText(Canvas canvas) {
        ...
        // }
        canvas.drawText(FormatTextPower(mCurrentBattery), mBatteryAhWidth * 0.5f + (mTextBounds.width() >> 4),
    ...
    private String FormatTextPower(float power) {
        String formatPower = power <= 0 ? "--" : String.format(Locale.getDefault(), "%2.0f%s", power, "%");
    ...
    public void setAh(int battery) {
        if (battery < 0 || battery > mFullBattery) return;
