...
public class EaseChatPrimaryMenu extends EaseChatPrimaryMenuBase implements View.OnClickListener {
    ...
    private static final int SEND_BTN_TYPE_ADD = 1;
    private static final int SEND_BTN_TYPE_MINUS = 2;
    private static final int SEND_BTN_TYPE_SEND = 3;
    ...
    public void onClick(View view) {
        ...
        if (id == R.id.btn_set_mode_voice) {
            ...
            hideVoiceMode();
            showMoreOrLessBtn(false);
            showNormalFaceImage();
            if (listener != null)
                listener.onToggleExtendClicked();
        } else if (id == R.id.btn_less) {
            showMoreOrLessBtn(true);
            if (listener != null)
                listener.onToggleExtendClicked();
    ...
    private void showMoreOrLessBtn(boolean isShowMore) {
        switchSendBtnType(isShowMore ? SEND_BTN_TYPE_ADD : SEND_BTN_TYPE_MINUS);
    }
    ...
    private void refreshEmojiSendBtn() {
    ...
    private void switchSendBtnType(int type) {
        buttonMore.setVisibility(GONE);
        buttonLess.setVisibility(GONE);
        buttonSend.setVisibility(GONE);
        switch (type) {
            case SEND_BTN_TYPE_ADD:
                buttonMore.setVisibility(VISIBLE);
                break;
            case SEND_BTN_TYPE_MINUS:
                buttonLess.setVisibility(VISIBLE);
                break;
            case SEND_BTN_TYPE_SEND:
                buttonSend.setVisibility(VISIBLE);
                break;
            default:
                throw new RuntimeException("Wrong send btn type");
        }
    }

    private void showMoreOrSendBtn() {
        if (editText.getText().length() > 0) {
            switchSendBtnType(SEND_BTN_TYPE_SEND);
        } else {
            switchSendBtnType(SEND_BTN_TYPE_ADD);
        }
    }
}