...
public class ChatRowVoice extends ChatRowFile {
    ...
    protected void onInflatView() {
        inflater.inflate(message.direct() == Message.Direct.RECEIVE ? R.layout.light_hd_row_received_voice : R.layout.light_hd_row_sent_voice, this);
    }
    ...
    private void playVoice(View v, String localPath, final boolean isSend) {
        ...
        if (((MessageAdapter) adapter).animView != null) {
            ...
            if (preIsSend) {
                ((MessageAdapter) adapter).animView.setBackgroundResource(R.drawable.chat_sender_audio_playing_4);
            } else {
                ((MessageAdapter) adapter).animView.setBackgroundResource(R.drawable.chat_receiver_audio_playing_4);
            }
        ...
        //播放音频
        MediaManager.playSound(getContext(), localPath, mp -> {
            ((MessageAdapter) adapter).currentPlayView = null;
            if (isSend) {
                ((MessageAdapter) adapter).animView.setBackgroundResource(R.drawable.chat_sender_audio_playing_4);
            } else {
