 ...
 *
 */
class WeekSleepHistogramViewEmptyTextAttr : SkinAttr() {

    override fun applySkin(view: View?) {
        val touchDailySleepHistogramView = view as? SleepHistogramView
        if (isColor) {
            touchDailySleepHistogramView?.setmEmptyTextColor(SkinResourcesUtils.getColor(attrValueRefId))
        }
    }
    ...
    override fun applyNightMode(view: View?) {
        ...
        val touchDailySleepHistogramView = view as? SleepHistogramView
        if (isColor) {
            touchDailySleepHistogramView?.setmEmptyTextColor(SkinResourcesUtils.getNightColor(attrValueRefId))
        }
    }
}