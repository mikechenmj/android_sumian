...
import com.sumian.hw.report.widget.SleepStateDurationView
 ...
 *
 */
class WeekSleepHistogramViewSoberTextAttr : SkinAttr() {

    override fun applySkin(view: View?) {
        val touchDailySleepHistogramView = view as? SleepHistogramView
        if (isColor) {
            touchDailySleepHistogramView?.setSoberColor(SkinResourcesUtils.getColor(attrValueRefId))
        }
    }
    ...
    override fun applyNightMode(view: View?) {
        ...
        val touchDailySleepHistogramView = view as? SleepHistogramView
        if (isColor) {
            touchDailySleepHistogramView?.setSoberColor(SkinResourcesUtils.getNightColor(attrValueRefId))
        }
    }
}