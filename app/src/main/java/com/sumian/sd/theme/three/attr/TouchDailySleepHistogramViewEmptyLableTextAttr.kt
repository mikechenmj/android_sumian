 ...
 *
 */
class TouchDailySleepHistogramViewEmptyLableTextAttr : SkinAttr() {

    override fun applySkin(view: View?) {
        val touchDailySleepHistogramView = view as? TouchDailySleepHistogramView
        if (isColor) {
            touchDailySleepHistogramView?.setEmptyLabelTextColor(SkinResourcesUtils.getColor(attrValueRefId))
        }
    }
    ...
    override fun applyNightMode(view: View?) {
        ...
        val touchDailySleepHistogramView = view as? TouchDailySleepHistogramView
        if (isColor) {
            touchDailySleepHistogramView?.setEmptyLabelTextColor(SkinResourcesUtils.getNightColor(attrValueRefId))
        }
    }
}