...
class SleepDiaryVpFragment : BaseFragment() {
    private val mAdapter by lazy { SleepDiaryPagerAdapter(fragmentManager!!) }

    companion object {
        private const val PRELOAD_THRESHOLD = 3
        private const val REQUEST_CODE_FILL_SLEEP_RECORD = 1
    }
    ...
    class SleepDiaryPagerAdapter(fragmentManager: FragmentManager) : FragmentPagerAdapter(fragmentManager) {
        ...
        override fun getItemPosition(obj: Any): Int {
            return if (obj is SleepDiaryFragment) {
                val positionByTime = times.indexOf(obj.initTime)
                positionByTime
            } else {
