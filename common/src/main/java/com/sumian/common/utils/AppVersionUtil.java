...
public final class AppVersionUtil {
    ...
    public static String getAppName(Context context) {
        ...
        return getPackageInfo(context).applicationInfo.loadLabel(packageManager).toString();
    }

    public static Drawable getAppLogo(Context context) {
        PackageManager packageManager = context.getPackageManager();
        return getPackageInfo(context).applicationInfo.loadLogo(packageManager);
    }

    private static void gotoMarket(Context context, String pck, String defaultApkDownloadUrl) {
        if (!isHaveMarket(context)) {
            ToastHelper.show(context, context.getString(R.string.none_market), Gravity.CENTER);
            // 打开应用商店失败 可能是没有手机没有安装应用市场
            // 调用系统浏览器进入商城
            openLinkBySystem(context, defaultApkDownloadUrl);
            return;
        }
    ...
    public static void openAppInMarket(Context context, String defaultApkDownloadUrl) {
        if (context == null) return;
        String pckName = context.getPackageName();
        gotoMarket(context, pckName, defaultApkDownloadUrl);
    }
